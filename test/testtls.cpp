...
//
// NOTE on the injection used: the task brief suggested testInjectEntityCertUnknown()
// to reach a "TcpOnly" gate.  That does NOT work on an already-started server: a
// started server's SSLContext is in TlsReady, and injecting UNKNOWN from TlsReady is
// the deliberate LIVE-UNKNOWN keep-alive path (SSLContext::setTlsOrTcpMode, openssl.cpp
// case UNKNOWN/TlsReady) -- it keeps TlsReady and merely pauses WRITE ops, so
// canRespondToTlsSearch() stays true and the TLS search still replies (verified:
