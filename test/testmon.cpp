...
struct BasicTest {
    ...
    // Regression: a monitor callback that cancels its own subscription (which
    // move-destroys the event functor) must not free the running functor or its
    // captures out from under itself.  doNotify() copies the callback before
    // invoking it, so the captured heap state stays valid for the whole call.
    void cancelDuringEvent()
    {
        ...
        self = cli.monitor("mailbox")
                ...
                .event([canary, fired, ok, &self](client::Subscription& s) {
                    try {
                        while(s.pop()) {}
                    } catch(...) {}
